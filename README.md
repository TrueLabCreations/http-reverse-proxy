...
The SimpleHTTPServer is a small implementation of a web server; it responds with the server number, 
host, name and url.
...
run locally. The certificates will be stored in the specified directory. 

The routes are configured to force an http connection from the browser to be redirected to an https
connection on the proxy. The certicates will be generated by the self signed LetsEncrypt service specified in the http options.

At startup the proxy will request the generation of the certificates for server1.test.com and server2.test.com. 
These certificates will be self-signed and will give you a warning in the browser. The certificates will be stored
...
The browser should be redirected to an https connection. This connection should display an error in the bowser 
...
__dnsNameServer__ | string | null | After writing the entry to the DNS table, the DNS challenge may verify the entry has been propogated within the cluster of name servers on the service before askting LetsEncrypt to look for it.
__log__ | object | null | The logging element
