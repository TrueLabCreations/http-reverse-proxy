...
__proxyOptions__ | object | See below | Options passed to the `node-http-proxy` instance used by this package. A complete list of the options can be found [here](https://github.com/http-party/node-http-proxy#options). Defaults below.
__httpsOptions__ | object | See below | The https interface options.
...
__letsEncryptOptions__ | object | See below | The Let's Encrypt server options.
...
Option | Type | Default | Description
---|---|---|---
__port__ | number | `443` | The inbound port used to listen for http connections.
__certificates__ | object | See Below | Certificate object, explained in detail below.
__host__ | network-address | http [hosts](https://nodejs.org/api/net.html#net_server_listen_port_host_backlog_callback) | The network interface to listen for http connections. This would only be used to force the system to listen on a single network. The format is a standard IPV4 or IPV6 network address. This has no relation to a host or hostname in a URL.
