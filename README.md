...
__httpsOptions__ | object | [See below](#https-server-options) | The https interface options.
...
caFilename | string | none | If the host for this route has a commercially generated certificate this should be the path and filename for the certificate authority file for the certificate. Including a `certificate authority` file at this level will completely override other well-known authorities. See more [here](https://nodejs.org/api/tls.html#tls_tls_createsecurecontext_options). If all of the certificates use a common authority, you can set the `ca` option in the [HttpsServerOptions](#https-server-options). This file reference is global. It is not relative to the certificateStoreRoot.
